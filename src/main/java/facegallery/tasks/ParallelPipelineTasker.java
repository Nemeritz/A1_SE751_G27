...
public class ParallelPipelineTasker {
    ...
    BlockingQueue<Integer> imageReaderOutput = new LinkedBlockingQueue<>();
    BlockingQueue<Integer> faceDetectInput = new LinkedBlockingQueue<>();
    BlockingQueue<Integer> thumbnailGenerateInput = new LinkedBlockingQueue<>();
    BlockingQueue<Integer> faceDetectOutput = new LinkedBlockingQueue<>();
    BlockingQueue<Integer> thumbnailGenerateOutput = new LinkedBlockingQueue<>();

    public ParallelPipelineTasker() {
        for (int i = 0; i < 4; i++) {
    ...
    private Void fileRead(Function<TaskerStats, Void> statsUpdater) {
        ...
        Void t = imageBytesReader.runAsync(imageReaderOutput);

        // Update the gui with each read for stats
        for (int i = 0; i < imageBytes.length; i++) {
            try {
                Integer m = imageReaderOutput.take();
                faceDetectInput.offer(m);
                thumbnailGenerateInput.offer(m);
    ...
    private Void thumbnailGenerate(Function<TaskerStats, Void> statsUpdater, Function<List<BufferedImage>, Void> imagesUpdater) {
        ...
        thumbnailGenerator.runAsyncPipeline(thumbnailGenerateInput, readyQueue);

        for (int i = 0; i < imageBytes.length; i++) {
            try {
                thumbnailGenerateOutput.offer(readyQueue.take());
            } catch (InterruptedException e) {
                e.printStackTrace(System.err);
            }
...
//            Void gs4 = imagesUpdater.apply(new CopyOnWriteArrayList<>(thumbnails));
    ...
    private Void faceDetect(Function<TaskerStats, Void> statsUpdater) {
        ...
        timer.startTiming();
        faceDetector.runAsyncPipeline(faceDetectInput, readyQueue);

        for (int i = 0; i < imageBytes.length; i++) {
            try {
                faceDetectOutput.offer(readyQueue.take());
            } catch (InterruptedException e) {
                e.printStackTrace(System.err);
            }
